...
static inline void rtl_sltu (rtlreg_t* dest, const rtlreg_t* src1, const rtlreg_t* src2) { 
    *dest = (*src1)<( *src2); 
  } 
...
static inline void rtl_pop(rtlreg_t* dest) {
  *dest = vaddr_read(cpu.esp, 4);// dest <- M[esp]
  cpu.esp +=4;// esp <- esp + 4
//  TODO();
}
...
static inline void rtl_update_ZF(const rtlreg_t* result, int width) {
  if(*result == 0)// eflags.ZF <- is_zero(result[width * 8 - 1 .. 0])
     t0 = 1;
  else
     t0 = 0;
  rtl_set_ZF(&t0);
...
static inline void rtl_update_SF(const rtlreg_t* result, int width) {
  t0 = result[width * 8 - 1];// eflags.SF <- is_sign(result[width * 8 - 1 .. 0])
  rtl_set_SF(&t0);
 // TODO();
}
